exports = module.exports = function(database, notification, spaces, csds, authentication, settings) {
...
router.post('/submitNotifyOptions',function(req, res, next){
    ...
    res.render('./notificationViews/notifyOptions',{"Result":result});
});
    ...
    router.get('/getR', function(req, res) {
        ...
        /*getRestrictions(function (restr) {
            ...
            res.render('./auth_views/authorization', hello);
        })*/
    ...
    function testIsAuth()
    {
        ...
        var obj = require('buzz-authentication')(settings, database, csds);
        obj.ServiceIdentifier("Authorization","updateAuthorizationRestriction");




        /*var checkAuth = authentication.please(settings, database, csds).isAuthorizedRequest("u12118282", sIdentifier);
        authentication.please(settings, database, csds).Authorization.isAuthorized(checkAuth);*/
    ...
    function testUpAuth()
    {
        ...
        var sIdentifier = authentication.please(settings, database, csds).ServiceIdentifier("Authorization","updateAuthorizationRestriction");
        var serviceRestriction= authentication.please(settings, database, csds).ServiceRestriction(2, sIdentifier);
        var authRestriction= authentication.please(settings, database, csds).AuthorizationRestriction(serviceRestriction,"COS 301","Student");
        var updateAuth= authentication.please(settings, database, csds).UpdateAuthorizationRestrictionRequest("u12118282",authRestriction);
        authentication.please(settings, database, csds).Authorization.updateAuthorisationRestriction(updateAuth);

    }

    function testAddAuth()
    {
        console.log('you there ');

        var sIdentifier = authentication(settings, database, csds).ServiceIdentifier("Authorization","updateAuthorizationRestriction");
    ...
    function testRemAuth()
    {
        console.log('you there remove ');

        var sIdentifier = authentication.please(settings, database, csds).ServiceIdentifier("Authorization", "removeAuthorizationRestriction");
        var serviceRestriction= authentication.please(settings, database, csds).ServiceRestriction(2, sIdentifier);
        var authRestriction= authentication.please(settings, database, csds).AuthorizationRestriction(serviceRestriction, "COS 301", "Student");
        var removeAuth= authentication.please(settings, database, csds).RemoveAuthorizationRestrictionRequest("u1223O83O", authRestriction);
        authentication.please(settings, database, csds).Authorization.removeAuthorizationRestriction(removeAuth);
